<cfscript>
  ...
  writeOutput("<div id='lvl_wrapper'></div>");
	        ...
	        var config = {
          	...
          	gapi.auth.authorize(config, function(authResult) {
          		...
          		if(typeof authResult.error === 'undefined'){  // no error during authorization
					...
					dteWrapperHTML = '<div style=\'color:red;\' id=\'lvl_error\' class=\'panel radius\'>** Lodge Calendar Error [AUTH] **</div>';
					jQuery('##lvl_wrapper').append(dteWrapperHTML); // append the panel
          		};
          	});
   };  // end init
	</script>
...
writeOutput("<script type='text/javascript'>
	function listUpcomingEvents() {
		...
		var request = gapi.client.calendar.events.list({
		  'calendarId': '#application.clubcalendarID#', 
			...
			//console.log(resp);
				...
				if(typeof resp.error === 'undefined'){  // NO ERROR
					...
					if (events.length > 0) {
						...
						dteWrapperHTML = '<div style=\'color:red;\' id=\'lvl_error\' class=\'panel radius\'>** Rental Calendar Error - No Events Found **</div>';
						jQuery('##lvl_wrapper').append(dteWrapperHTML); // append the panel
					}
				} // end NO CALENDAR ERROR

				else {  // CALENDAR ERROR
					calendarErrorMsg = '** Rental Calendar Error [' + resp.code + ']-' + resp.message + ' **';
					dteWrapperHTML = '<div style=\'color:red;\'id=\'lvl_error\' class=\'panel radius\'>' + calendarErrorMsg + '</div>';
					jQuery('##lvl_wrapper').append(dteWrapperHTML); // append the panel
				}
...
writeOutput("
	<script type='text/javascript'>
		...
		function outputEvent(eventItem,cntr) {  // put out the events html
			...
			dataWrapperID = 'lvldataWrapper_' + cntr;
			dteInfoID = 'lvldateInfo_' + cntr;
			dteDetailID = 'lvldateDetail_' + cntr;
...
");
writeOutput("<script type='text/javascript' src='https://apis.google.com/js/client.js?onload=addLoadEvent(func(){initLevelClub}'>// This must stay here</script>");

writeOutput("
<div class='panel radius hide-for-small'>
<h6 class='subheader'>
		<a href='http://www.google.com/calendar/embed?src=#application.clubcalendarID#' target='_blank'>VIEW the complete Rental Calendar powered by <img align='absmiddle' src='images/google.gif' alt='google image'></a>
</h6>
</div>
	");
